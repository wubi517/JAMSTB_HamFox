...
import com.gold.kds517.homFox_newui.adapter.DateListAdapterExtra;
import com.gold.kds517.homFox_newui.adapter.EpgListAdapterExtra;
...
import com.gold.kds517.homFox_newui.listner.SimpleGestureFilter.SimpleGestureListener;
import com.gold.kds517.homFox_newui.models.EpgModel;
...
public class GuideActivityExtra extends AppCompatActivity implements AdapterView.OnItemClickListener,View.OnClickListener,SimpleGestureListener,
      IVLCVout.Callback{
    ...
    DateListAdapterExtra dateListAdapter;
    EpgListAdapterExtra epgListAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        epg_date = Constants.year_dateFormat.format(new Date());
        page = 0;
...
//        }
                    ...
                    if(jsonArray.length() > 0){
                        ...
                        runOnUiThread(()->{
                            if(epgModelList.size()>0){
                                ...
                                dateListAdapter = new DateListAdapterExtra(this,date_datas);
    ...
    private void printEpgData(){
        ...
        epgListAdapter = new EpgListAdapterExtra(this,date_epgModel);
    ...
    private String getFromDate(long millisecond){
        ...
        String formattedDate=Constants.guide_format.format(date);
    ...
    private String getFromCatchDate(long millisecond){
        Date date = new Date();
        date.setTime(millisecond);
        String formattedDate=Constants.catchupFormat.format(date);
        return formattedDate;
    }
    ...
    private String getFromDate1(long millisecond){
        Date date = new Date();
        date.setTime(millisecond);
        String formattedDate=Constants.time_format.format(date);
        return formattedDate;
    }
...
//            vout.setSubtitlesView(tv_subtitle);
    ...
    private static class MediaPlayerListener implements MediaPlayer.EventListener {
        private WeakReference<GuideActivityExtra> mOwner;

        public MediaPlayerListener(GuideActivityExtra owner) {
            mOwner = new WeakReference<GuideActivityExtra>(owner);
        }

        @Override
        public void onEvent(MediaPlayer.Event event) {
            GuideActivityExtra player = mOwner.get();
...
//                    Toast.makeText(player, "Error", Toast.LENGTH_SHORT).show();
    ...
    private void showSubTracksList() {
        AlertDialog.Builder builder = new AlertDialog.Builder(GuideActivityExtra.this);
